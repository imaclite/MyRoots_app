...
/**
 * "نسب" هو تطبيق واحد مخصّص (وليس منصّة متعددة التطبيقات)، لذا الاسم والأيقونات
 * والألوان ثابتة هنا بدل الاشتقاق من اسم المضيف — بذلك يبقى المانفست صحيحًا
 * على أي استضافة (Vercel، نطاق خاص، أو حتى المعاينة المحلية)، لا فقط على
 * نطاقات *.grok.me. `hostHeader` غير مستخدم الآن ويبقى في التوقيع فقط
 * لتوافق الاستدعاءات القائمة.
 */
export function renderWebManifest(hostHeader) {
  void hostHeader;
  const name = "نسب — شجرة العائلة";
  return JSON.stringify(
    {
      id: "/",
      name,
      short_name: "نسب",
      description: "شجرة عائلة عربية مع حفظ محلي واستيراد/تصدير GEDCOM 5.5.1.",
